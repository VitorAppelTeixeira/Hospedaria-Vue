  ...
  methods: {
    ...
    async fazerLogin() {
      ...
      try {
        ...
        const msg = e.response?.data?.erro || e.response?.data?.detail || 'Usuario ou senha invalidos.'
        alert(msg)
      } finally {
    ...
    async fazerCadastro() {
      ...
      if (!this.username || !this.email || !this.password) {
        alert('Informe usuario, e-mail e senha.')
        return
      }
      try {
        this.carregando = true
        ...
        const msg = e.response?.data?.erro || e.response?.data?.detail || 'Nao foi possivel criar o cadastro. Verifique se o backend esta rodando em ' + (import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000')
        alert(msg)
      } finally {
        this.carregando = false
      }
...
<template>
  <div class="login-container">
    ...
    <button class="botao-secundario" @click="alternarModo" :disabled="carregando" type="button">
      {{ modoFormulario === 'cadastro' ? 'Ja tenho cadastro' : 'Criar cadastro' }}
    </button>
  </div>
</template>

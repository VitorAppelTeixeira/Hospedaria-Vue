  ...
  methods: {
    async carregarOpcoes() {
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel carregar as opcoes.'))
      }
    },
    ...
    async salvar() {
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel salvar os dados.'))
      } finally {
        this.salvando = false
      }
    },
...
<template>
  <div class="container">
    <CaixaTexto label="Titulo" v-model="titulo" />
    <CaixaTexto label="Descricao" v-model="descricao" multiLinha />
    ...
    <CaixaTexto label="Preco da diaria (R$)" v-model="precoDiaria" tipo="number" />

  ...
  methods: {
    async carregar() {
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel carregar as hospedagens.'))
      } finally {
    ...
    async excluir(id) {
      ...
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel excluir a hospedagem.'))
      }
    },
    editar(item) {
    ...
    reservar(item) {
      if (!this.hospedeId) {
        alert('Nao foi possivel identificar o hospede logado.')
        return
      }
      this.$router.push({
        name: 'CriarReserva',
  ...
  <div class="container">
    ...
    <template v-else>
      ...
      <div v-for="item in hospedagens" :key="item.id" class="card">
        ...
        <p>{{ item.descricao }}</p>
        <p>Tipo: {{ item.tipo }} | Diaria: R$ {{ item.preco_diaria }}</p>
        <p>Capacidade: {{ item.capacidade }} | Quartos: {{ item.quartos }} | Banheiros: {{ item.banheiros }}</p>
        <p>Endereco: {{ descreverEndereco(item.endereco) }}</p>

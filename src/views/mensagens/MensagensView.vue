  ...
  methods: {
    async carregar() {
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel carregar as mensagens.'))
      } finally {
    ...
    async excluir(id) {
      ...
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel excluir a mensagem.'))
      }
    },
    responder(item) {
      this.$router.push({
  ...
  <div class="container">
    ...
    <template v-else>
      ...
      <div v-for="item in mensagens" :key="item.id" class="card">
        ...
        <p>E-mail: {{ item.email }} | Telefone: {{ item.telefone || 'Nao informado' }}</p>
        <p>Assunto: {{ item.assunto }}</p>
        <p>{{ item.mensagem }}</p>
        <p>Lida: {{ item.lida ? 'Sim' : 'Nao' }}</p>

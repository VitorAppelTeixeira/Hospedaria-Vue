  ...
  methods: {
    ...
    async carregar() {
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel carregar as reservas.'))
      } finally {
    ...
    async excluir(id) {
      ...
      try {
        ...
        alert(mensagemErro(e, 'Nao foi possivel excluir a reserva.'))
      }
    },
    editar(item) {
  ...
  <div class="container">
    ...
    <template v-else>
      ...
      <div v-for="item in reservas" :key="item.id" class="card">
        ...
        <p>Hospedagem: {{ item.hospedagem }} | Hospede: {{ item.hospede }}</p>
        <p>Check-in: {{ isoParaBr(item.data_checkin) }} | Check-out: {{ isoParaBr(item.data_checkout) }}</p>
        <p>Hospedes: {{ item.quantidade_hospedes }} | Total: R$ {{ item.valor_total }}</p>
